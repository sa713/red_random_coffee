...
После каждой успешной жеребьёвки бот автоматически отправляет администраторам в личку сводку:
- дата;
- номер завершённого раунда (по порядку);
- сколько человек участвовало в этом раунде;
- топ-3 участников по количеству уникальных пар за всё время.
...
- За `READY_WINDOW_HOURS` до жеребьёвки приходит сообщение с кнопками «Да, участвую» и «Нет, пропущу».
- «Да, участвую» фиксирует участие в ближайшем раунде.
- «Нет, пропущу» фиксирует пропуск раунда и отправляет ответ «Хорошо! Новый раунд через неделю, напишу!».
